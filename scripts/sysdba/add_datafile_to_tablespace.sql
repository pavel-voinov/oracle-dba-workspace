...
Script to add datafile(s) to tablespace in ASM diskgroups/NAS storage/LION stanardized environments with required number of autoextended datafiles

Usage: @add_datafile_to_tablespace.sql <tablespace name> <base directory/diskgroup for datafiles|default|null|*|none|any> <number of datafiles to create>
...
begin
  ...
  WHERE t.name = l_ts
    AND t.ts# = d.ts#
    AND regexp_like(d.name, '^/s[0-9]{2}/oradata[0-9]{1}/');
  is_lion := i > 0;
  l_last_number := i;
  ...
  l_SQL := 'ALTER TABLESPACE "' || l_ts || '" ADD DATAFILE';
  i := 1;
  while i <= l_cnt
  loop
    if i > 1 then
