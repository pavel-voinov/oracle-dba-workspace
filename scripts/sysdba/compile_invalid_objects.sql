...
declare
  l_owner varchar2(30) := nvl(upper(trim('&1')), sys_context('USERENV', 'CURRENT_SCHEMA'));
  l_last integer := -1;
  l_now integer;
  ...
  begin
    ...
    SELECT count(*) into l_cnt FROM dba_objects WHERE owner = l_owner AND status = 'INVALID';
