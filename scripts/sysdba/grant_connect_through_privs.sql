/*
Script to grant privileges for proxy connection for all non-system and non-special users
*/
...
begin
  ...
  for u in (SELECT username
            ...
            WHERE not regexp_like(username, :v_sys_users_regexp)
              AND not regexp_like(username, '^(&proxy_user)$', 'i'))
  loop
    begin
      dbms_output.put_line('ALTER USER "' || u.username  || '" GRANT CONNECT THROUGH "&proxy_user"' || ';');
      execute immediate 'ALTER USER "' || u.username  || '" GRANT CONNECT THROUGH "&proxy_user"';
