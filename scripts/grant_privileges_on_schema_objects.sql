...
Script to grant ALL/RO (read-only) or REVOKE privileges on db objects in current schema to specified user
*/
...
set termout off
column db_owner new_value db_owner
SELECT sys_context('USERENV', 'CURRENT_SCHEMA') as db_owner FROM dual;
set termout on
...
begin
  ...
  for x in (select 'GRANT select, insert, update, delete ON "' || t.owner || '"."' || table_name || '" TO "' || l_user || '"' as grant_sql,
              ...
              table_name,
              TEMPORARY,
              (select count(*) from user_tab_privs p where p.owner = t.owner and grantee = l_user and p.table_name = t.table_name) as is_exists
            from all_tables t
  ...
  for x in (select 'GRANT select ON "' || t.owner || '"."' || t.object_name || '" TO "' || l_user || '"' as grant_select_sql,
              ...
              (select count(trigger_name) from all_triggers s where s.owner = t.owner AND s.table_name = t.object_name and s.trigger_type = 'INSTEAD OF') as triggers_count,
	            (select count(*) from user_tab_privs p where p.owner = t.owner AND p.grantee = l_user AND p.table_name = t.object_name) as is_exists
            FROM all_objects t
  ...
  for x in (select distinct 'GRANT execute ON "' || t.owner || '"."' || type_name || '" TO "' || l_user || '"' as grant_sql,
              ...
              (select count(*) from user_tab_privs p where p.owner = t.owner AND p.grantee = l_user and p.table_name = t.type_name) as is_exists
            from all_types t
            where t.owner = l_owner) loop
    if l_mode = 'REVOKE' AND x.is_exists > 0 then
      exec_SQL(x.revoke_sql);
  ...
  for x in (select 'GRANT select ON "' || t.sequence_owner || '"."' || sequence_name || '" TO "' || l_user || '"' as grant_sql,
              ...
              'REVOKE all ON "' || t.sequence_owner || '"."' || sequence_name || '" FROM "' || l_user || '"' as revoke_sql,
	            (select count(*) from user_tab_privs p where p.owner = t.sequence_owner AND p.grantee = l_user and p.table_name = t.sequence_name) as is_exists
