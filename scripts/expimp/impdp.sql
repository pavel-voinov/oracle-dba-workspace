...
begin
  ...
  dbms_datapump.metadata_remap(handle => l_handle, name => 'REMAP_TABLESPACE', old_value => l_old_schema || '_IDX', value => l_data_ts);
  dbms_datapump.metadata_remap(handle => l_handle, name => 'REMAP_TABLESPACE', old_value => l_old_schema || '_LOG', value => l_data_ts);
  dbms_datapump.metadata_remap(handle => l_handle, name => 'REMAP_TABLESPACE', old_value => l_old_schema || '_DOC', value => l_data_ts);
  dbms_datapump.metadata_remap(handle => l_handle, name => 'REMAP_TABLESPACE', old_value => l_old_schema || '_LOB', value => l_data_ts);
  ...
  dbms_datapump.metadata_remap(handle => l_handle, name => 'REMAP_TABLESPACE', old_value => 'JPHARM_PUBLIC', value => l_data_ts);
  dbms_datapump.metadata_remap(handle => l_handle, name => 'REMAP_TABLESPACE', old_value => 'IDX_TREE_DATA', value => l_data_ts);
  dbms_datapump.metadata_remap(handle => l_handle, name => 'REMAP_TABLESPACE', old_value => 'JPHARM_SM_DATA', value => l_data_ts);
  dbms_datapump.metadata_remap(handle => l_handle, name => 'REMAP_TABLESPACE', old_value => 'JPHARM_SM_LOB', value => l_data_ts);
  dbms_datapump.metadata_remap(handle => l_handle, name => 'REMAP_TABLESPACE', old_value => 'NG_DATA', value => l_data_ts);
  dbms_datapump.metadata_remap(handle => l_handle, name => 'REMAP_TABLESPACE', old_value => 'GVDB_DATA', value => l_data_ts);
  ...
  dbms_datapump.metadata_filter(handle => l_handle, name => 'EXCLUDE_PATH_EXPR', value => 'IN(''AUDIT_OBJ'',''DB_LINK'',''JOB'',''GRANT'',''REFRESH_GROUP'',''DOMAIN_INDEX'')');
