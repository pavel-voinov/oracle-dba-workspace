/*
*/
set serveroutput on size unlimited echo off feedback on timing off
...
PROMPT Already disabled primary keys (before)
...
/
PROMPT
PROMPT Still enabled primary keys (after)
SELECT owner, table_name, constraint_name
FROM all_constraints
WHERE owner = sys_context('USERENV', 'CURRENT_SCHEMA')
  AND regexp_like(table_name, '^&table_name.$', 'i')
